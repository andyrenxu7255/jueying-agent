...
- [ ] SQL 查询使用参数化或 ORM
- [ ] 文件上传有类型、大小、内容验证
- [ ] 飞书长连接配置不误要求 webhook signing secret；所有 token/secret 在管理台状态接口中只显示是否已配置或掩码值
- [ ] ClawHub Admin Token 不进入 Git、不进入 skill metadata、不通过 `/api/admin/clawhub/status` 回显
- [ ] ClawHub 导入/升级只接受规整后的安全 slug（`A-Za-z0-9._-`，拒绝 `--help`、路径跳转、空 slug），并通过参数数组调用 CLI/HTTP，避免 shell 展开和选项注入
- [ ] 认证和授权逻辑正确实现
- [ ] 错误信息不泄露系统内部细节
