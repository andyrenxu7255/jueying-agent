...
| DEV-22 | [用户视角全链路测试设计](./DEV-22-用户视角全链路测试设计.md) | 七类角色、14条用户旅程、自动化与人工UAT映射 | 用户故事线 / AH1-30 / context-graph |
...
- [ ] DEV-22 中 UJ-01 ~ UJ-14 全部执行，自动化项 100% 通过
- [ ] `npm run test:user-journey-coverage` 通过，确认角色、入口、脚本和图谱均覆盖
- [ ] 外部凭据型 UAT（真实飞书/企微、正式 LLM、生产可观测后端）已在目标环境记录结论
- [ ] 任何失败项必须修复并复验，不允许以“部分通过”标记主版本准出
