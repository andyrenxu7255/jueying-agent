...
The sync module implements an event-sourcing layer for OpenCode session replayability. It enables one device to control and modify a session while multiple other devices can replay the event log to stay in sync.

> **tl;dr** All of these APIs work, are properly type-checked, and sync events are backwards compatible with `Bus`:
...
The following sync events are defined in `src/sync/index.ts`:

| Event | Direction | Description |
|-------|-----------|-------------|
| `sync:start` | Client → Server | Request to start syncing a workspace |
| `sync:replay` | Client → Server | Replay sync events from a specific point |
| `sync:history` | Client → Server | Request sync event history |
| `sync:event` | Server → Client | A synced event payload |

Each event type uses a typed schema defined via `busSchema`. See `src/sync/index.ts` for the current schema definitions.

## Type safety for busSchema

The conversion done in `convertEvent` is not automatically type-checked with `busSchema`. To ensure they stay in sync:

1. **Add a round-trip test**: Encode an event with `busSchema`, decode it, and assert the output matches the input.
2. **Use `Schema.asserts`**: After any schema change, run `bun test` to verify that existing events still parse correctly.
3. **Consider generating `convertEvent` from `busSchema`**: If the mapping becomes complex, auto-generate the converter from the schema definition to eliminate manual drift.
