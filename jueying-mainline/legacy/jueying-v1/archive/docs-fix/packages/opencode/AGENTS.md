...
- Use `Effect.gen(function* () { ... })` for composition. Do not wrap a single `Service.use(...)` call in `Effect.gen(...)` just to return it — use the direct form instead.
- Use `Effect.fn("Domain.method")` for named/traced effects and `Effect.fnUntraced` for internal helpers.
- Inside `Effect.fn`, use `function*` when you need to `yield*` other Effects; use a plain arrow function when the body is a single Effect (e.g. `Effect.succeed(...)`) or does not compose other Effects.
...
- Use `Schema.Class` for top-level structured data that needs a named type in the SDK/OpenAPI spec (e.g. request/response bodies, entities with identity). See `specs/effect/http-api.md` for the full decision matrix.
- Use `Schema.Struct` for nested, anonymous, or internal data shapes that do not need a named `$ref` in the generated spec. This avoids SDK type bloat.
...
## Terminology: Instance, InstanceState, InstanceRef

| API | Status | When to use |
|-----|--------|-------------|
| `Instance` (from `src/effect/instance-registry.ts`) | **Legacy** — do not add new usages | ALS helper for reading `Instance.directory`, `Instance.current`, `Instance.provide`. Being phased out. |
| `InstanceState` (from `src/effect/instance-state.ts`) | **Current** — preferred for new code | Per-directory state with automatic cleanup via `ScopedCache`. Use for services that need separate state per open project. |
| `InstanceRef` (from `src/effect/instance-ref.ts`) | **Future** — eventual replacement for `Instance` | Effect-native context tag. Migration in progress; see `specs/effect/instance-context.md`. |

Migration direction: `Instance.*` → `InstanceState.*` (short-term) → `InstanceRef` (long-term).
...
### InstanceState API reference

`InstanceState` (from `src/effect/instance-state.ts`) exports the following:

| API | Description |
|-----|-------------|
| `InstanceState.make<A, E, R>(init: (ctx: InstanceContext) => Effect<A, E, R \| Scope>)` | Create a per-directory state. The `init` callback receives an `InstanceContext` with `directory`, `workspaceID`, etc. |
| `InstanceState.get(state)` | Get the current directory's state (resolves via `ScopedCache`). |
| `InstanceState.use(state, selector)` | Get and project a property from the current state. |
| `InstanceState.useEffect(state, effect)` | Get the state and run an Effect against it. |
| `InstanceState.has(state)` | Check if state exists for the current directory. |
| `InstanceState.invalidate(state)` | Invalidate the cached state, forcing re-initialization on next access. |
| `InstanceState.bind(fn)` | ALS binding for native callbacks (wraps `Instance.bind` with fallback). |
| `InstanceState.context` | Get the current `InstanceContext`. |
| `InstanceState.directory` | Get the current instance directory path. |
| `InstanceState.workspaceID` | Get the current workspace ID. |
...
## InstanceState.bind — ALS for native callbacks

`InstanceState.bind(fn)` (exported from `src/effect/instance-state.ts`) captures the current Instance AsyncLocalStorage context and restores it synchronously when called. It wraps `Instance.bind` with `LocalContext.NotFound` fallback.
