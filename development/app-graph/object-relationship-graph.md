...
网络隔离（2026-05-11 审计修复后）:
  ┌─────────────────────────────────────────────────────────────────┐
  │  agent-harness-net (172.28.0.0/16) — 应用通信网络               │
  │  承载: 全部应用服务 + LiteLLM + SigNoz                           │
  └─────────────────────────────────────────────────────────────────┘
  ┌─────────────────────────────────────────────────────────────────┐
  │  agent-harness-backend (172.29.0.0/16, internal) — 数据库存储   │
  │  承载: PostgreSQL + Redis + MinIO + ClickHouse                   │
  │  应用服务通过双网络接入访问后端存储                                │
  └─────────────────────────────────────────────────────────────────┘
```
