...
> 版本: 2026-05-04 (第十一轮：文件存储机制 + 用户隔离空间 + 全部文档审计完成)
> 当前状态: **全链路验证通过，TypeScript编译零错误，可投产使用**

---

## 一、系统概述

agent-harness（品牌名 JueYing / 绝影）是一个 AI Agent 编排与执行平台。用户通过飞书/企微等 IM 渠道与系统交互，系统使用 LLM 将用户意图规划为多阶段工作流，自动调度执行器完成各阶段任务，最终汇报结果。系统支持多租户、用户隔离、策略控制、记忆管理。
...
### fact-retrieval — 文件存储端点（新增）

| 方法 | 路径 | 用途 |
|------|------|------|
| GET | `/api/files` | 列出当前用户的文件（支持 category/scope 过滤） |
| POST | `/api/files/upload` | 上传文件（base64 编码） |
| GET | `/api/files/:id/download` | 下载文件 |
| POST | `/api/files/:id/share` | 修改文件 scope（private/shared/public） |
